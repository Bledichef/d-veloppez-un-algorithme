...
function displayAllRecipes(recipes) {
  ...
  for (const recipe of recipes) {
    ...
    tagArticle.ariaLabel =
      "Recettes filtrées suite à la recherche dans la barre de recherche et/ou dans les tags";
 ...
 */
function displayTag() {
  const allElementsIngredients = document.querySelectorAll(
    ".element-ingredient"
  );
  ...
  const allElements = [
    allElementsIngredients,
    allElementsDevices,
    allElementsUstensils,
  ];
  ...
  for (const element of allElements) {
    element.forEach((el) => {
      el.addEventListener("click", (e) => {
        ...
        const containerInputIng = document.querySelector(
          ".container-input-ing"
        );
        ...
        const containerInputDev = document.querySelector(
          ".container-input-dev"
        );
        ...
        const containerInputUst = document.querySelector(
          ".container-input-ust"
        );
...
function displaySelectDevice(recipes) {
  ...
  const containerInputDevice = document.querySelector(
    ".container-input-devices"
  );
...
function displaySelectIngredients(recipes) {
  ...
  const containerInputIngredient = document.querySelector(
    ".container-input-ingredients"
  );
...
function displaySelectUstensils(recipes) {
  ...
  const containerInputUstensil = document.querySelector(
    ".container-input-ustensils"
  );
    ...
    item.addEventListener("click", () => {
      ...
      for (const tagIngredient of tagsIngredients) {
        if (item.id === tagIngredient.id) {
          //si l'id de l'élément cliqué est le même que l'id de la croix cliqué -> on retire du dom la balise
  ...
  allRecipes = recipes.filter((recipe) =>
    tagSelect.every((tag) => {
      ...
      for (const itemUstensil of recipe.ustensils) {
        ...
        for (const tagUstensil of tagsUstensils) {
          if (
            ustensil.includes(tag.toLowerCase()) &&
            tagUstensil.id === ustensil
          ) {
            filterUstensil = true;
          }
        }
      ...
      for (const itemIngredient of recipe.ingredients) {
        ...
        for (const tagIngredient of tagsIngredients) {
          if (
            ingredient.includes(tag.toLowerCase()) &&
            tagIngredient.id === ingredient
          ) {
      ...
      for (const tagDevice of tagsDevices) {
        if (
          ...
          tagDevice.id === recipe.appliance.toLowerCase()
        ) {
          return true;
        }
      }
...
//Remplacer une des boucles par Array.prototype.includes
//Liens ci joint
  ...
  searchBar.addEventListener("input", (e) => {
    ...
    if (valueInput.length > 2) {
      ...
      for (const recipe of recipes) {
        ...
        if (
          description.includes(valueInput) &&
          !newArrayRecipes.includes(recipe)
        ) {
          newArrayRecipes.push(recipe);
        }
        ...
        for (const ingredient of ingredients) {
          if (
            ingredient.ingredient.toLowerCase().includes(valueInput) &&
            !newArrayRecipes.includes(recipe)
          ) {
            newArrayRecipes.push(recipe);
          }
