...
function sortRecipesByTag(recipes) {
  ...
  const tagsUstensils = Array.from(document.querySelectorAll(".tag-ustensils")).map(tag => tag.id.toLowerCase());
  const tagsDevices = Array.from(document.querySelectorAll(".tag-device")).map(tag => tag.id.toLowerCase());
  const tagsIngredients = Array.from(document.querySelectorAll(".tag-ingredients")).map(tag => tag.id.toLowerCase());
  allRecipes = recipes.filter(recipe =>
    tagsUstensils.every(tag =>recipe.ustensils.includes(tag)
    ) 
     &&
     tagsDevices.every(tag => recipe.appliance.toLowerCase()===tag)
    //   recipe.appliance.toLowerCase().includes(tag) && tagsDevices.includes(recipe.appliance.toLowerCase())
    // ) 
    &&
     tagsIngredients.every(tag => recipe.ingredients.some(ingredient => ingredient.ingredient.toLowerCase()===tag
      )
    //   recipe.ingredients.some(ingredient =>
    //     ingredient.ingredient.toLowerCase().includes(tag) && tagsIngredients.includes(ingredient.ingredient.toLowerCase())
    //   )
    )
  );
